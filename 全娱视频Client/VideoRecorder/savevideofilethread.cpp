...
//void SaveVideoFileThread::slot_writeVideoFrameDate(uint8_t *picture_buf, int buffer_size)
//{
...
int SaveVideoFileThread::write_frame(AVFormatContext *fmt_ctx, AVCodecContext *c, AVStream *st, AVFrame* frame)
{
    ...
    if(ret < 0)
    {
        fprintf(stderr, "Error send a frame to the encoder\n");
        exit(1);
    }
    while(ret >= 0)
    {
...
void SaveVideoFileThread::slot_setInfo(STRU_AV_FORMAT& av)
{
    m_avFormat = av;

    std::string fileName = m_avFormat.fileName.toStdString();
    avformat_alloc_output_context2(&oc, NULL, "flv", fileName.c_str());//保存flv格式
    //为了未来推流rtmp(flv)
    if (!oc) {
...
const STRU_AV_FORMAT &SaveVideoFileThread::avFromat() const
{
    return m_avFormat;
}
